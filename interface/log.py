...
def init_logger(name = None, path = None, logname = None, mode = 'a', level='ERROR:DEBUG',
    ...
    path = path if path is not None else os.path.join(os.path.dirname(os.path.abspath(__file__)), 'logs')
    if not os.path.exists(path):
        os.makedirs(path)
    logname = logname if logname is not None else 'log.txt'
    limit = limit if limit is not None else 1024 * 1024 * 10
    backup_countbackup_count = 10 if backup_count == 0 else backup_count
    filepath = os.path.join(path, logname)

    add_streamhandler(s_level, FORMAT)
    add_filehandler(f_level, FORMAT, filepath, mode, backup_count, limit, when)
...
init_logger(name='test', limit=100*100)
iCount = 0
