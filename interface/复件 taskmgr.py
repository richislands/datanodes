...
class task:
    ...
    def __init__(self, exe_path, exe_params):
        ...
        raises:
        '''
        self.exe_path, self.exe_params = exe_path, exe_params
    ...
    def start_task(self):
        ...
        try:
            ...
            if os.path.exists(self.exe_path):
                ...
                self.handle = common.start_exe('%s %s' % (self.exe_path, self.exe_params), file_out, file_error)
        ...
        for tsk in task_list:
            try:
                if tsk.get_status_last_time() > 0:
                    ...
                    flag = tsk.stop_taskmgr()
                    if flag == 1:
                        stop_count += 1
                        log.info('程序[%s]停止成功' % tsk.name)
                    else:
                        log.error('程序[%s]停止失败,原因:%s' % repr(ce))
                    time.sleep(1)
            except custom_exception as ce:
...
def status_machine():
    ...
    class _service_status:
        ...
        def refresh_service_status(self):
            ...
            if self._bindService and self._bindData:
